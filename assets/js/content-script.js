...
chrome.storage.local.get(
  ...
  async function (items) {
    ...
    // Apply settings based on stored values
    const sidebarValue = items[CONFIG_KEYS.SIDEBAR];
    if (sidebarValue === 0) {
      showSidebar();
    } else if (sidebarValue === 1) {
      hideSidebar();
    }

    if (items[CONFIG_KEYS.BLUR_NAMES] === 1) BlurNames();
    if (items[CONFIG_KEYS.BLUR_PHOTOS] === 1) BlurPhotos();
    if (items[CONFIG_KEYS.BLUR_RECENT_MESSAGES] === 1) BlurRecentMessages();
    if (items[CONFIG_KEYS.BLUR_CONVERSATION_MESSAGES] === 1) BlurConversationMessages();
    
    if (items[CONFIG_KEYS.DARK_THEME] === 1) {
      EnableDarkTheme();
    } else {
      DisableDarkTheme();
    }
...
function getElementByXpath(path) {
  ...
  if (element && element.outerHTML.includes("menu")) {
...
function Opacity() {
  chrome.storage.local.get(
    [
      "whatsapp_config_blur_names",
      "whatsapp_config_blur_photos",
      "whatsapp_config_blur_recent_messages",
      "whatsapp_config_blur_conversation_messages",
      "whatsapp_config_opacity",
    ],
    async function (items) {
      blur_opacity = items["whatsapp_config_opacity"];
      if (items["whatsapp_config_blur_names"] == 1) BlurNames(true);
      if (items["whatsapp_config_blur_photos"] == 1) BlurPhotos(true);
...
function hideSidebar() {
  ...
  if (sideElement && sideElement.parentElement) {
    const parentElement = sideElement.parentElement;
    parentElement.style.display = "none";
    
    const hideButton = document.getElementById("stateHide");
    const showButton = document.getElementById("stateShow");
    
    if (hideButton) hideButton.style.display = "none";
    if (showButton) showButton.style.display = "";
    
    chrome.storage.local.set({ [CONFIG_KEYS.SIDEBAR]: 1 });
  }
}
...
function showSidebar() {
  ...
  if (sideElement && sideElement.parentElement) {
    ...
    chrome.storage.local.set({ [CONFIG_KEYS.SIDEBAR]: 0 });
  }
}
...
function BlurNames(opacity = false) {
  if (!document.getElementById("BlurNames") || opacity) {
    ...
    let element = document
      .querySelector("#pane-side > div > div > div")
      .querySelectorAll(":scope > div");
    if (element.length > 0) {
      var css_class = element[0].querySelector('[role="gridcell"] div')
        .classList[0];
  ...
  } else document.getElementById("BlurNames").remove();
}
...
function BlurConversationMessages(opacity = false) {
  if (!document.getElementById("BlurConversationMessages") || opacity) {
    ...
    style.innerHTML = `
			...
			`;
    document.getElementsByTagName("head")[0].appendChild(style);
  } else document.getElementById("BlurConversationMessages").remove();
}
function BlurPhotos(opacity = false) {
  if (!document.getElementById("BlurPhotos") || opacity) {
    ...
    let element = document
      ...
      .querySelectorAll(":scope > div");
    if (element.length > 0) {
      var css_class = element[0]
        .querySelector('[data-testid="cell-frame-container"]')
        .querySelector(":scope > div > div").classList[0];
      var header = "#main > header > div > div";
      var style = document.createElement("style");
      style.setAttribute("id", "BlurPhotos");
      style.innerHTML = `.${css_class}, ${header}
			{
  ...
  } else document.getElementById("BlurPhotos").remove();
}
...
function BlurRecentMessages(opacity = false) {
  if (!document.getElementById("BlurRecentMessages") || opacity) {
    ...
    let element = document
      ...
      .querySelectorAll(":scope > div");
    if (element.length > 0) {
      var css_class = element[0].querySelector(
        '[data-testid="cell-frame-container"] div:nth-child(2) > div:nth-child(2) > div'
      ).classList[0];
      var style = document.createElement("style");
  ...
  } else document.getElementById("BlurRecentMessages").remove();
}



function initialization() {
  var icon_hide = getElementByXpath(
    '//*[@id="app"]/div/div/div[3]/header/div[2]/div/span'
  );
  icon_hide.prepend(
...
function initStorage() {
  chrome.storage.local.get(
    ...
    function (items) {
      ...
      // Initialize all config values with defaults if undefined
      ...
      // Special handling for blur_names based on theme
      const themeFromLocalStorage = localStorage.getItem("theme");
      if (items[CONFIG_KEYS.BLUR_NAMES] === undefined || themeFromLocalStorage === "light") {
        updates[CONFIG_KEYS.BLUR_NAMES] = 0;
      } else if (themeFromLocalStorage === "dark") {
        updates[CONFIG_KEYS.BLUR_NAMES] = 1;
      }
