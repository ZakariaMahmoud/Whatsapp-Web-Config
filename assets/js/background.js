chrome.runtime.onInstalled.addListener(() => {
	chrome.storage.local.get(["whatsapp_config_sidebar", "whatsapp_config_blur_names", "whatsapp_config_blur_photos", "whatsapp_config_blur_recent_messages", "whatsapp_config_blur_conversation_messages", "whatsapp_config_dark_theme"], async function (items) {
		if (items["whatsapp_config_sidebar"] === undefined) chrome.storage.local.set({ "whatsapp_config_sidebar": 0 })
		if (items["whatsapp_config_blur_names"] === undefined) chrome.storage.local.set({ "whatsapp_config_blur_names": 0 })
